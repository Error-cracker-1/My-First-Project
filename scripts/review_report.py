...
def _format_file_change_details(file_changes: list[dict[str, str]] | None) -> list[str]:
    """
    Format detailed file changes (Which, What, Why) for Markdown report output.
    """
    if not file_changes:
        return ["- None"]
...
def _format_code_diffs(modified_files: list[str]) -> list[str]:
    """
    Format git diffs per file for Markdown report output.
    """
    ...
    for f in modified_files:
        ...
        diff_text = ""
        try:
            res = subprocess.run(["git", "diff", "HEAD", "--", f], capture_output=True, text=True)
            diff_text = res.stdout.strip()
            if not diff_text:
                res = subprocess.run(["git", "diff", "HEAD~1", "HEAD", "--", f], capture_output=True, text=True)
                diff_text = res.stdout.strip()
            if not diff_text:
        ...
        if diff_text:
            ...
            lines.append("*No git diff captured for file.*")
        lines.append("")
    return lines
...
def generate_review_report(
    ...
    lines = [
        ...
        *_format_file_list(modified_files),
        ...
        "",
        *_format_file_change_details(file_change_details),
        "",
        "## Code Diffs",
        "",
        *_format_code_diffs(modified_files),
        "",
        "## Skipped Files",
        "",
